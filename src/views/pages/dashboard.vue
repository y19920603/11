<template>
  ...
  <div class="p-4 pt-0 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-2">
    <DashBoardCardItem v-for="item in dashBoardCards" :item="item" />
  </div>
  <div
    class="xl:w-1/2 h-[650px] m-8 rounded-xl border-2 border-[#eee] shadow-md dark:bg-[#2c2e3b] dark:border-none"
  >
...
import { useLoadingStore, useTagsViewStore } from "@/store";
...
// 查询
const gameProportions = ref<GameProportion[]>([]);
  ...
  DashBoardAPI.getDashBoardData(queryParams)
    .then(({ result, data }) => {
      if (result) {
        ...
        gameProportions.value = data.game_proportions;
...
watch(
  () => settingsStore.theme,
  () => {
    if (gameProportions.value.length) {
      updateChartOptions(gameProportions.value);
    }
  }
);
